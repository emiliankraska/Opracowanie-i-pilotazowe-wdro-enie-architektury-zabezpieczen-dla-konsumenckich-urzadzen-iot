...
sqlite3_close(db);
  ...
  coap_add_resource(ctx, resource);
    resource = coap_resource_init(coap_make_str_const("attestation_request"), 0);
  coap_register_handler(resource, COAP_REQUEST_GET,
      [](auto, auto, const coap_pdu_t *request, auto, coap_pdu_t *response) {
          coap_show_pdu(COAP_LOG_WARN, request);
          
          // Generate a new nonce
          auto nonce = generate_attestation_nonce();
          
          // Convert nonce to hex string for transmission
          char nonce_hex[33]; // 16 bytes = 32 hex chars + null terminator
          for (size_t i = 0; i < nonce.size(); i++) {
              sprintf(&nonce_hex[i * 2], "%02x", nonce[i]);
          }
          
          coap_pdu_set_code(response, COAP_RESPONSE_CODE_CONTENT);
          coap_add_data(response, 32, reinterpret_cast<const uint8_t*>(nonce_hex));
          
          coap_log_info("Generated nonce: %s\n", nonce_hex);
          coap_show_pdu(COAP_LOG_WARN, response);
      });
  coap_add_resource(ctx, resource);
  ...
  // Modified attestation handler with secret data retrieval
  resource = coap_resource_init(coap_make_str_const("attestation"), 0);
  coap_register_handler(resource, COAP_REQUEST_PUT, 
      [](auto, auto, const coap_pdu_t *request, auto, coap_pdu_t *response) {
          ...
          if (incoming_pcr == pcr_value) {
              // Match found - query secret data from database
              sqlite3 *db = nullptr;
              ...
              if (sqlite3_open("mydb.db", &db) != SQLITE_OK) {
                  ...
                  coap_add_data(response, 14, (const uint8_t *)"Database Error");
              ...
              sqlite3_stmt *stmt;
              std::string sql = "SELECT secret_key, private_note FROM DaneWrazliwe WHERE id = '1';";
              
              if (sqlite3_prepare_v2(db, sql.c_str(), -1, &stmt, nullptr) == SQLITE_OK) {
                  if (sqlite3_step(stmt) == SQLITE_ROW) {
                      const unsigned char* key_text = sqlite3_column_text(stmt, 0);
                      const unsigned char* note_text = sqlite3_column_text(stmt, 1);
                      
                      secret_key = key_text ? reinterpret_cast<const char*>(key_text) : "N/A";
                      private_note = note_text ? reinterpret_cast<const char*>(note_text) : "N/A";
                  } else {
                      secret_key = "Not Found";
                      private_note = "Not Found";
                  }
              } else {
                  std::cerr << "SQL prepare failed: " << sqlite3_errmsg(db) << "\n";
                  secret_key = "Query Error";
                  private_note = "Query Error";
              }
              
              sqlite3_finalize(stmt);
              sqlite3_close(db);
              
              // Format response with secret data
              std::string response_msg = "Attestation Completed\nSecret Key: " + secret_key + "\nPrivate Note: " + private_note;
              
              coap_pdu_set_code(response, COAP_RESPONSE_CODE_CHANGED);
              coap_add_data(response, response_msg.length(), 
                           reinterpret_cast<const uint8_t*>(response_msg.c_str()));
              
              coap_log_info("Attestation successful. Sent secret data.\n");
          } else {
              // No match
              coap_pdu_set_code(response, COAP_RESPONSE_CODE_BAD_REQUEST);
              coap_add_data(response, 29, (const uint8_t *)"No Match, Attestation Failed");
              
              coap_log_warn("Attestation failed. PCR mismatch.\n");
          }
